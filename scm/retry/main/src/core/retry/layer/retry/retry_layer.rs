...
use super::RetryErrorClassifier;
...
#[cfg(test)]
impl RetryLayer {
    /// Should the given outcome trigger a retry? (Test helper.)
    ///
    /// `Ok(status)` → retry when the status code is in the retryable list.
    /// `Err(is_transient)` → retry when the error is classified as transient.
