//! Fluent builder for constructing [`AwsSigV4StrategyConfig`] instances.

use secrecy::SecretString;

use crate::api::error::AuthError;
pub use crate::api::types::strategy::aws_sig_v4_strategy_config::AwsSigV4StrategyConfig;
...
#[derive(Default)]
pub struct AwsSigV4StrategyConfigBuilder {
...
impl AwsSigV4StrategyConfigBuilder {
    ...
    pub fn build_config(self) -> Result<AwsSigV4StrategyConfig, AuthError> {
        let access_key_id = self.access_key_id.ok_or_else(|| {
            AuthError::InvalidHeaderValue(
                "AwsSigV4StrategyConfigBuilder: access_key_id is required".into(),
            )
        })?;
        let secret_access_key = self.secret_access_key.ok_or_else(|| {
            AuthError::InvalidHeaderValue(
                "AwsSigV4StrategyConfigBuilder: secret_access_key is required".into(),
            )
        })?;
        let region = self.region.ok_or_else(|| {
            AuthError::InvalidHeaderValue(
                "AwsSigV4StrategyConfigBuilder: region is required".into(),
            )
        })?;
        let service = self.service.ok_or_else(|| {
            AuthError::InvalidHeaderValue(
                "AwsSigV4StrategyConfigBuilder: service is required".into(),
            )
        })?;
